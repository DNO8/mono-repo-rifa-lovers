...
const lucky_pass_service_1 = require("../lucky-pass/lucky-pass.service");
...
describe('LuckyPassService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                lucky_pass_service_1.LuckyPassService,
                { provide: lucky_pass_repository_1.LuckyPassRepository, useValue: mockLuckyPassRepository },
                { provide: raffles_repository_1.RafflesRepository, useValue: mockRafflesRepository },
            ],
        }).compile();
        service = module.get(lucky_pass_service_1.LuckyPassService);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('findById', () => {
        ...
        it('should throw NotFoundException if pass not found', async () => {
            ...
            await expect(service.findById('invalid')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    ...
    describe('markAsWinner', () => {
        ...
        it('should throw if updated pass not found after marking', async () => {
            ...
            mockLuckyPassRepository.findUnique.mockResolvedValue(null);
            await expect(service.markAsWinner('pass-1')).rejects.toThrow(common_1.NotFoundException);
        });
    });
});
//# sourceMappingURL=lucky-passes.service.spec.js.map