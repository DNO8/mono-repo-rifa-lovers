...
describe('PurchasesService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ...
                { provide: purchases_repository_1.PurchasesRepository, useValue: mockPurchasesRepository },
                { provide: packs_repository_1.PacksRepository, useValue: mockPacksRepository },
                { provide: raffles_repository_1.RafflesRepository, useValue: mockRafflesRepository },
            ],
        }).compile();
        service = module.get(purchases_service_1.PurchasesService);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe('confirmPayment', () => {
        it('should confirm payment and update status', async () => {
            ...
            mockPrisma.$queryRaw
                .mockResolvedValueOnce([{ id: 'raffle-1' }])
                .mockResolvedValueOnce([{ max_ticket: '0' }]);
        ...
        it('should throw if purchase not found', async () => {
            ...
            await expect(service.confirmPayment('invalid-id', {
                providerTransactionId: 'flow-123',
                provider: 'flow',
                status: 'approved',
            })).rejects.toThrow(common_1.NotFoundException);
...
//# sourceMappingURL=purchases.service.spec.js.map